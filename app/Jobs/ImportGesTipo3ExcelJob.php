...
use App\Mail\GesTipo3ImportResumenMail;
...
class ImportGesTipo3ExcelJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (!$jobRow) {
            Log::error("ImportGesTipo3ExcelJob: no existe import_jobs id={$this->importJobId}");
            return;
        }
        ...
        try {
            $this->updateJob($jobRow, 'running', 5, 'inicio', 'Iniciando importacion gestantes tipo 3...');
            ...
            $t0 = microtime(true);
            $import = new GesTipo3Import($this->userId, $batchId);
            ...
            Log::error('ImportGesTipo3ExcelJob ERROR: ' . $e->getMessage(), [
    ...
    private function rollbackBatchSafe(int $batchId): void
    {
        ...
        try {
            ...
            Log::error('ImportGesTipo3ExcelJob cleanup ERROR: ' . $cleanupError->getMessage(), [
                'batchId' => $batchId,
            ]);
        }
    }
    ...
    private function sendImportEmailSafe(
        ...
        try {
            ...
            if (!$to) {
                Log::warning('GesTipo3 mail: no hay destinatarios validos', [
                    'userId' => $this->userId,
                    'batchId' => $batchId,
                ]);
                return;
            ...
            $mail = new GesTipo3ImportResumenMail(
            ...
            Log::error('GesTipo3 mail ERROR: ' . $mailError->getMessage(), [
                'userId' => $this->userId,
                'batchId' => $batchId,
            ]);
        }
