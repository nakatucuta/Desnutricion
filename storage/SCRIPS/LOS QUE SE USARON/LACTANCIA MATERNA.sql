...
SELECT 
    ...
    CAST(R1.fecconlac AS date)											AS fechaConsulta,
    ''																	AS codigoConsulta,
    'Atención Para la Promoción y Apoyo a la Lactancia Materna'			AS descrip,
...
WHERE    
    ...
    -- Filtros pedidos:
	AND R1.fecconlac > '1900-01-01'
    AND FN.fechaNac IS NOT NULL
    AND (
        DATEDIFF(YEAR, FN.fechaNac, @Hoy) 
        - CASE WHEN DATEADD(YEAR, DATEDIFF(YEAR, FN.fechaNac, @Hoy), FN.fechaNac) > @Hoy THEN 1 ELSE 0 END
    ) BETWEEN 0 AND 5
ORDER BY Afi.numeroCarnet, R1.fecconlac
