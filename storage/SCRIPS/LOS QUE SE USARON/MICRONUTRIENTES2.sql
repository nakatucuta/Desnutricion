...
SELECT 
    ...
    CAST(F.fechaInicio AS date)							AS fechaProcedimiento,
    A.codigoServicio									AS codigoProcedimiento,
...
INNER JOIN sga..ripsaf F
	ON A.codigoIps = F.codigoIps AND A.numeroFactura=F.numeroFactura
...
WHERE 
    A.codigoServicio IN ('V06DX')
    AND F.fechaInicio >= @Desde
    AND F.fechaInicio <  @HastaExclusivo

UNION ALL

-- ======== SEGUNDA PARTE (ripsnAP) ========
SELECT 
	...
	afi.primerApellido									AS primerApellido,
	afi.segundoApellido									AS segundoApellido,
    CAST(A.fechaSuministroTecnologia AS date)           AS fechaConsulta,
    A.codTecnologiaSalud                                AS codigoConsulta,
...
WHERE 
    A.codTecnologiaSalud IN ('V06DX')
    AND A.fechaSuministroTecnologia >= @Desde
    AND A.fechaSuministroTecnologia <  @HastaExclusivo;
