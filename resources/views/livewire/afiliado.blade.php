...
@section('content')
    ...
    @endif --}}
        <div class="row">
            <div class="col-md-12">
                <form action="{{ route('import-excel_2') }}" method="POST" enctype="multipart/form-data"
                    class="form-horizontal" id="file-upload-form">
                    @csrf
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).ready(function() {
                    ...
                    if (query.length > 0) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                $('#search-results')
                            .empty(); // Limpiar resultados previos
                            ...
                            error: function(xhr, status, error) {
                                $('#loading-spinner')
                            .hide(); // Ocultar el spinner si hay un error
                ...
                $(document).on('click', '.search-result-item', function(e) {
                    ...
                    $('#search').val(
                    numeroIdentificacion); // Establece el valor en el campo de búsqueda
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {
                            search: numeroIdentificacion
                        },
                        success: function(response) {
                            $('#sivigila tbody').html(''); // Limpiar la tabla
                            $('#loading-spinner').hide(); // Ocultar el spinner

                            $.each(response.sivigilas_usernormal, function(index,
                                student2) {
                                var correoEnviado = student2.correo_enviado ?
                                    true : false;

                                var acciones = correoEnviado ?
                                    '<button class="btn btn-sm btn-secondary" disabled><i class="fas fa-envelope"></i> Correo Enviado</button>' :
                                    '<a href="#" class="btn btn-sm btn-warning blinking-button send-email" data-toggle="modal" data-target="#emailModal" data-id="' +
                                    student2.id + '" data-name="' + student2
                                    .primer_nombre + ' ' + student2
                                    .segundo_nombre + ' ' + student2
                                    .primer_apellido + ' ' + student2
                                    .segundo_apellido +
                                    '"><i class="fas fa-envelope"></i> Solicitud</a>';

                                $('#sivigila tbody').append(
                                    '<tr>' +
                                    '<td>' + student2.id + '</td>' +
                                    '<td><a href="#" class="numero-identificacion" data-id="' +
                                    student2.id + '">' + student2
                                    .numero_identificacion + '</a></td>' +
                                    '<td>' + student2.primer_nombre + ' ' +
                                    student2.segundo_nombre + ' ' + student2
                                    .primer_apellido + ' ' + student2
                                    .segundo_apellido + '</td>' +
                                    '<td>' + student2.numero_carnet +
                                    '</td>' +
                                    '<td>' + acciones + '</td>' +
                                    '</tr>'
                                );
                            ...
                            $('#search-results')
                        .empty(); // Limpiar los resultados de la lista desplegable

                            handleEmailModal
                        (); // Adjuntar manejador para el botón de "Solicitud"
                            attachEventHandlers();
                        },
                        error: function(xhr, status, error) {
                            $('#loading-spinner')
            ...
            function attachEventHandlers() {
                $('.numero-identificacion').on('click', function(e) {
                        ...
                        var url =
                            '{{ route('getVacunas', ['id' => ':id', 'numero_carnet' => ':numeroCarnet']) }}';
        ...
        attachEventHandlers(); table.on('draw', function() {
            attachEventHandlers();
        });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (currentDate >= startDate && currentDate <= endDate) {
                ...
                dragDropArea.innerHTML =
                    "<p>El formulario no está disponible fuera del rango de fechas permitido.</p>";
            }
        ...
        function handleEmailModal() {
            ...
            $('#sendEmailButton').off('click').on('click', function() {
                ...
                $('.send-email[data-id="' + patientId + '"]').prop('disabled', true).removeClass('blinking-button')
                    .addClass('btn-secondary').html('<i class="fas fa-envelope"></i> Correo Enviado');
